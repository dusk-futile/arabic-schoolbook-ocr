...
W, H = 1080, 530
PAPER_W, PAPER_H = 330, 320
DOC_W, DOC_H = 380, 360
LEFT_X, RIGHT_X = 56, 636
TOP = 62
...
def messy_sheet(seed: int, tilt: float) -> Image.Image:
    css = ("*{font-family:sans-serif;font-size:11.5px;direction:rtl;"
           "text-align:justify;line-height:1.9;color:#242830;}")
    body = arabic(f"<div dir='rtl'>{BROKEN}</div>", css, PAPER_W - 34, PAPER_H - 54)
    sheet = Image.new("RGB", (PAPER_W, PAPER_H), (250, 247, 240))
    sheet.paste(body, (17, 34))
...
def word_doc(reveal: float = 1.0) -> Image.Image:
    css = ("*{font-family:sans-serif;direction:rtl;color:#1a1e26;}"
           "h2{font-size:14.5px;text-align:right;font-weight:bold;"
           "margin:0 0 13px 0;line-height:1.55;}"
           "div.p{font-size:12px;text-align:justify;line-height:2.05;"
           "margin:0 0 13px 0;}")
    html = "".join(
        (f"<h2 dir='rtl'>{t}</h2>" if k == "h" else f"<div class='p' dir='rtl'>{t}</div>")
        for k, t in CLEAN)
    body = arabic(html, css, DOC_W - 52, DOC_H - 92)

    doc = Image.new("RGB", (DOC_W, DOC_H), (255, 255, 255))
    doc.paste(body, (26, 74))
    d = ImageDraw.Draw(doc)
    # Word-ish title bar so it reads as a .docx, not "some page"
    d.rectangle([0, 0, DOC_W - 1, 34], fill=(43, 87, 154))
    d.rectangle([8, 8, 26, 26], fill=(255, 255, 255))
    d.text((13, 11), "W", fill=(43, 87, 154), font=font(13, True))
    d.text((36, 11), "book.docx", fill=(255, 255, 255), font=font(12, True))
    d.line([(0, 34), (DOC_W, 34)], fill=(33, 70, 128))
    d.rectangle([0, 0, DOC_W - 1, DOC_H - 1], outline=(214, 220, 230), width=1)
    if reveal < 1.0:
        cut = int(74 + (DOC_H - 84) * reveal)
...
def compose(reveal: float = 1.0, pulse: float = 1.0) -> Image.Image:
    ...
    f_lab, f_small, f_title, f_stat = font(16), font(13), font(23, True), font(15, True)
...
def main() -> None:
    ...
    for i in range(n):
        ...
        frames.append(compose(min(1.0, max(0.0, (t - 0.12) / 0.58)),
                              min(1.0, t / 0.72)))
    frames += [frames[-1]] * 8
    pal = [f.convert("P", palette=Image.ADAPTIVE, colors=80) for f in frames]
